        ...
        public async Task<string> HandleAsync(Usuario usuario, ChatSession sesion, string mensaje)
        {
            ...
            switch (mensaje)
            {
                ...
                case "2":
                    ...
                    respuesta = $"Entre al siguiente link para registrar el proyecto: https://kmtzj8db-5010.usw3.devtunnels.ms/FormulariosSupervisor/FormularioCrearProyecto?token={sesion.TokenFormularios}";
                ...
                case "7":
                    ...
                    foreach (var item in empleadosConTareas)
                    {
                        respuesta += $"Id: {item.empleado.Id}\n" +
                            ...
                            $"Telefono: {item.empleado?.Usuario?.PhoneNumber ?? "Sin registrar"}\n" +
